...
@section('content')
    ...
    <form action="{{ route('product.update', ['product' => $product->id]) }}" enctype="multipart/form-data" method="POST" class="row gy-4">
        @csrf
        @method('PUT')
        ...
        <div class="col-md-3">
            ...
            <input name="sale_price" type="text" value="{{ $product->sale_price > 0 ? $product->sale_price : '' }}" class="form-control" />
            <div class="form-text text-danger">
                @error('sale_price')
                    {{ $message }}
                @enderror
