...
@section('content')
    ...
    <form action="{{route('coupon.store')}}" method="POST" class="row gy-4">
        ...
        <div class="col-md-3">
           ...
           <input name="percentage" type="text" value="{{old('percentage')}}" class="form-control" />
            <div class="form-text text-danger">@error('percentage') {{$message}}@enderror</div>
        </div>
         <div class="col-md-3">
            <label class="form-label">تاریخ انقضا</label>
           <input data-jdp name="expired_at" type="text" value="{{old('expired_at')}}" class="form-control" />
